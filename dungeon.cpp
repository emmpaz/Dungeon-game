...
void rotateVec(double angle, double *v, double *arr)
{ //rotates a vectore ccw
	double cs = cos(angle);
	double sn = sin(angle);
	*arr = (v[0] * cs) + (v[1] * sn);
...
void printBoard(dungeon *d)
{
	...
	{
		for (int k = -1; k < 2; k += 2)
		{ //manually going through direct N,S,E,W
			for (int l = 0; l <= 1; l++)
			{
				for (int i = 0; i <= VEIW_RADIUS; i++)
				{
					...
					if (row < ROWS && col < COLS && row >= 0 && col >= 0)
					{
						...
						else
						{
							...
							{ //adding monsters if they are in seen location
		...
		for (double a = theta; a < 6.5; a += theta)
		{
			...
			int prevCol = 0;
			bool noWall = true;
			for (int l = 0; l < den && noWall; l++)
			{ //if vect broken up repeat
				for (i = 0; abs(i) < (abs(checkSightVec[0])) / (den - l) && noWall; checkSightVec[0] > 0 ? i++ : i--)
				{
					for (j = 0; abs(j) < (abs(checkSightVec[1])) / (den - l) && noWall; checkSightVec[1] > 0 ? j++ : j--)
					{ //loops through vecter and breaks if touches a wall, has a bais
		...
		for (int i = 0; i < ROWS; i++)
		{ //copy revelaedBoard into a temp board
			for (int j = 0; j < COLS; j++)
			{
	...
	else
	{
		...
		{ //replacing terrain with characters
...
void printBoardWcursor(dungeon *d, int cursorRow, int cursorCol)
{
	...
	board.board[cursorRow][cursorCol] = '*';
	for (int i = 0; i < ROWS; i++)
	{ //printing board
		for (int j = 0; j < COLS; j++)
		{
...
heap_t movePrioQueueInit(dungeon *d)
{ //init prioQueue
	heap_t h;
...
//displaying monsters on new window
...
int nextTurn(dungeon *d, heap_t *h)
{
	...
	if (c->nextTurn <= d->turn && c->dead != 1)
	{ //if a charactor has a turn on the current turn
		if (c->typeofChar != '@')
		{
			monster_move(c, d);
			for (int i = 1; i < d->numOfCharacters; i++)
...
void game_loop(dungeon *d, heap_t *h)
{
	...
	bool win = false;
	while ((d->characters)->dead != 1 && d->monstersAlive > 0 && !win)
	{
		nextTurn(d, h);
...
void getMonstDescrip(std::vector<MonsterDescription> &v)
{
	...
	if (name != "RLG327 MONSTER DESCRIPTION 1") //making sure its a description file
		return;
	while (monsterFile)
	{ //while the text file still has lines
